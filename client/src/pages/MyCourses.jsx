...
const ProgressBar = ({ value }) => (
  <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5" role="progressbar" aria-valuenow={value} aria-valuemin={0} aria-valuemax={100}>
    <div
...
const StatCard = ({ icon: Icon, iconColor, label, value }) => (
  <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center text-center gap-1">
  ...
  return (
    <StudentShell>
      <div className="py-8 px-4 sm:px-6">
        ...
        {/* ── Search + filter bar ─────────────────────────────────────────────── */}
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 px-4 py-3 mb-6 flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-100 bg-slate-50 dark:bg-slate-900 text-sm text-slate-700 dark:text-slate-300 placeholder-slate-300 outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent hover:border-slate-200 dark:border-slate-700 transition-all"
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            {filtered.map(({ course, originalIndex, progress }) => {
              ...
              return (
                <div
                  key={course.id}
                  className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow duration-300 flex flex-col"
                >
                  {/* Coloured banner header */}
                  <div className={`bg-gradient-to-br ${gradient} relative h-36 p-4 flex flex-col justify-between`}>
                    <div className="flex justify-between items-start">
                      {/* Category pill */}
                      <span className="px-3 py-1 bg-white/90 dark:bg-slate-900/20 backdrop-blur-sm text-slate-900 dark:text-white text-xs font-semibold rounded-full capitalize">
                        {course.category}
                      </span>
                      {/* Completion badge */}
                      {isComplete && (
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-400 mb-5">
                      ...
                      <span className="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-full capitalize">
                        {course.level}
                      </span>
                    </div>
                    ...
                    <button
                      ...
                      className={[
                        ...
                        isComplete
                          ? 'bg-green-50 dark:bg-green-900/20 text-green-600 hover:bg-green-100 border border-green-100 focus:ring-green-400'
            ...
            <button
              onClick={() => navigate('/explore-courses')}
              className="rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 p-6 flex flex-col items-center justify-center text-center hover:border-blue-300 hover:bg-blue-50 dark:bg-blue-900/20 transition-all group min-h-[280px] focus:outline-none focus:ring-2 focus:ring-blue-400"
              aria-label="Explore more courses"
            >
              <div className="w-14 h-14 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-4 group-hover:bg-blue-100 transition-colors">
                <FaCompass className="text-xl text-slate-300 group-hover:text-blue-500 transition-colors" />
              </div>
              <h4 className="font-bold text-slate-500 dark:text-slate-400 text-sm mb-1 group-hover:text-blue-500 transition-colors">
                Explore More
              </h4>
